...
class PetNotifier extends StateNotifier<AsyncValue<List<Pet>>> {
  ...
  Future<void> addPet(Pet pet) async {
    final failureOrSuccess = await _petRepository.addPet(pet);
    state = failureOrSuccess.fold(
      (failure) => AsyncValue.error(failure.message, StackTrace.current),
      (_) => const AsyncValue.data([]),
    );
    await fetchPets(); // Refresh the pets list after adding a pet
  }

  Future<void> updatePet(String id, Pet pet) async {
    final failureOrSuccess = await _petRepository.updatePet(id, pet);
    state = failureOrSuccess.fold(
      (failure) => AsyncValue.error(failure.message, StackTrace.current),
      (_) => const AsyncValue.data([]),
    );
    await fetchPets(); // Refresh the pets list after updating a pet
  }
  ...
  Future<void> deletePet(String id) async {
    ...
    await fetchPets(); // Refresh the pets list after deleting a pet
  }
}
